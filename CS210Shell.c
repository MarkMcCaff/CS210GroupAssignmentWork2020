...
void loadHistory() {
	...
	char commandStr[512]; 
	//doesn't run the rest of the method if the file cannot be found and prints an error
	if (fptr == NULL) {
		printf("-> the file does not exist - no history loaded.\n");
		return;
	}
	//loops for the maximum number of elements possible in historyArray
	for (int i = 0; i < 20; i++) {
...
//load from .aliases into aliases
void loadAlias() {
	...
	if (fptr == NULL) {
		printf("-> the file does not exist - no aliases loaded.");
		return;
	}
